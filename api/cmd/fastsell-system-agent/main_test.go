...
func TestEvaluateDockerHealthMatchesExpectedContainerNameAlias(t *testing.T) {
	...
	addDockerServiceAliases(servicesByName, models.SystemDockerService{
		...
		ContainerName: "fastsell_postgres",
		State:         "running",
		Health:        "healthy",
		Status:        models.SystemStatusOK,
	})

	health := evaluateDockerHealth([]string{"postgres"}, servicesByName)
	...
	if health.Services[0].ContainerName != "fastsell_postgres" {
		t.Fatalf("expected container name fastsell_postgres, got %q", health.Services[0].ContainerName)
	}
}

func TestEvaluateDockerHealthReportsTrulyMissingService(t *testing.T) {
	health := evaluateDockerHealth([]string{"postgres"}, map[string]models.SystemDockerService{})
	...
	if !strings.Contains(health.Alerts[0].Message, "Expected Docker service is missing: postgres.") {
